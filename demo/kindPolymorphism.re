...
class virtual category ('f) = {
  ...
  pub virtual compose:
    'a 'b 'c .
    (app('b, app('a, 'f)), app('c, app('b, 'f))) => app('c, app('a, 'f))
  ;
};
...
let category_fun = {
  ...
  pub compose = (f, g) => Fun.inj((x) => Fun.prj(g, Fun.prj(f, x)))
};
...
let category_ip = {
  ...
  pub compose = (f, g) => Ip.inj({ip: (x) => Ip.prj(g).ip(Ip.prj(f).ip(x))})
...
let category_cat_left = (_: #category('f)) => {
  ...
  pub compose:
    'a 'b 'c .
    (app('b, app('a, app('f, CL.t))), app('c, app('b, app('f, CL.t)))) =>
    app('c, app('a, app('f, CL.t)))
   =
    (type a, type b, type c) => (
      (type a, type b, type c, f: app(b, app(a, app('f, CL.t))), j: app(c, app(b, app('f, CL.t)))) =>
        CL.(
          switch (prj(j): cat_left(c, b, 'f)) {
          | Ident => (f: app(c, app(a, app('f, CL.t))))
          | [@implicit_arity] Compose(g, h) =>
            inj([@implicit_arity] Compose(prj(self#compose(f, inj(g))), h))
          }
        ):
        (app(b, app(a, app('f, CL.t))), app(c, app(b, app('f, CL.t)))) =>
        app(c, app(a, app('f, CL.t)))
...
let rec observe: type f a b. (#category(f), cat_left(b, a, f)) => app(b, app(a, f)) =
  (cat) =>
    fun
    | Ident => cat#ident
    | [@implicit_arity] Compose(f, g) =>
      cat#compose(observe(cat, f), g); /* Lift a value into cat_left. */

let promote: type f a b. app(b, app(a, f)) => cat_left(b, a, f) =
  (c) => [@implicit_arity] Compose(Ident, c);