...
let monad_c () => {
  ...
  pub return a => C.inj {c: fun h => h a};
  pub bind = {
    let bind {c: p} k => {c: fun h => p (fun a => (k a).c h)};
    fun m k => C.inj (bind (C.prj m) (fun a => C.prj (k a)))
  }
};

/* rep :: Monad m => m a -> C m a */
let rep: 'a 'm .#monad 'm => app 'a 'm => c 'a 'm = fun o m => {c: fun k => o#bind m k};

/* abs :: Monad m => C m a -> m a */
let abs: 'a 'm .#monad 'm => c 'a 'm => app 'a 'm = fun o c => c.c o#return;
...
let monad_free (functor_free: #functor_ 'f) => {
  ...
  pub bind = {
    let rec bind m k =>
      switch m {
      | Return a => k a
      | Wrap t => Wrap (functor_free#fmap (fun m => bind m k) t)
      };
...
let freelike_c (f_functor: #functor_ 'f) (freelike: #freelike 'f 'm) => {
  ...
  pub wrap t => C.inj {c: fun h => freelike#wrap (f_functor#fmap (fun p => (C.prj p).c h) t)}
};

type freelike_poly 'a 'f = {fl: 'm 'd .(#freelike 'f 'm as 'd) => app 'a 'm};

/* improve :: Functor f => (forall m. FreeLike f m => m a) -> Free f a */
let improve: 'a 'f .#functor_ 'f => freelike_poly 'a 'f => free 'a 'f =
  fun d {fl} => Free.prj (abs (monad_free d) (C.prj (fl (freelike_c d (freelike_free d)))));
...
let functor_f_io = {
  ...
  pub fmap h l =>
    F_io.inj (
      ...
      | PutChar c x [@implicit_arity] => PutChar c (h x) [@implicit_arity]
      }
    )
};
...
let putChar: 'm .#freelike F_io.t 'm => char => app unit 'm =
  fun f c => f#wrap (F_io.inj (PutChar c (f#mm#return ()) [@implicit_arity]));
...
let rec run: 'a .free 'a F_io.t => list char => output 'a =
  fun f cs =>
    ...
    | Wrap x =>
      ...
      | PutChar c p [@implicit_arity] => Print c (run p cs) [@implicit_arity]
      }
    };

/* run revEcho stream */
let simulate_original stream => run (Free.prj (revEcho (freelike_free functor_f_io))) stream;

/* run (improve revEcho) stream */
let simulate_improved stream => run (improve functor_f_io {fl: fun d => revEcho d}) stream;
